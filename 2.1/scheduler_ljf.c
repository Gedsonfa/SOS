...
// Scheduler function implementing the LJF algorithm
struct proc * scheduler(struct proc * current) {
    ...
    if (isempty(ready)) {
        // printf("== sched LJF: READY queue is empty\n");
        return NULL;
    }

    // Implementing LJF strategy:
    struct proc * temp = ready->head;
    int max_remaining_time = -1;

    while (temp != NULL) {
        // Select the process with the maximum remaining time
        if (temp->remaining_time > max_remaining_time) {
            max_remaining_time = temp->remaining_time;
            selected = temp;
        }
